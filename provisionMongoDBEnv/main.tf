...
### Network ###
  ...
  ### Network :: Shared Network ###
    ...
    resource "opc_compute_security_application" "mongodb-27017" {
      name     = "mopngodb-27017"
      protocol = "tcp"
      dport    = "27017"
    }
    ...
    resource "opc_compute_sec_rule" "my-sec-rule-2" {
      depends_on       = ["opc_compute_security_list.my-sec-list-1", "opc_compute_security_application.mongodb-27017"]
      name             = "${var.prefix}_my-sec-rule-2"
      source_list      = "seciplist:${opc_compute_security_ip_list.my-sec-ip-list-1.name}"
      destination_list = "seclist:${opc_compute_security_list.my-sec-list-1.name}"
      action           = "permit"
      application      = "mopngodb-27017"
    }    
