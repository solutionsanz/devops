#######################################################################
# Provision OIC and DBCS via PSM CLI commands
...
if [ "$#" -ne 7 ]; then
    ...
    echo " Usage: [oicPassword DBAPassword WLPassword prefix idDomainName]"
    echo "****************************************"
    exit 1
...
sed -i "s/@APP_NAME_GOES_HERE@/${app}/g" templates/temp_psm_IntegrationCloud_Dev_DBCS.json
# Setting Identity Domain Name to setup Storage API Path:
sed -i "s/@ID_DOMAIN_NAME_GOES_HERE@/${idDomainName}/g" templates/temp_psm_IntegrationCloud_Dev_DBCS.json
...
# Setting Environment App Name:
sed -i "s/@APP_NAME_GOES_HERE@/${app}/g" templates/temp_psm_IntegrationCloud_Dev.json
# Setting Identity Domain Name to setup Storage API Path:
sed -i "s/@ID_DOMAIN_NAME_GOES_HERE@/${idDomainName}/g" templates/temp_psm_IntegrationCloud_Dev.json

#######################################################################
#######################################################################
#################### Calling PSM to provision DBCS and OIC

psm dbcs create-service -c templates/temp_psm_IntegrationCloud_Dev_DBCS.json > templates/temp_${app}_psm_dbcs.out
dbcsJobId="NA"
...
echo "****************************************"
echo " DBCS Provisioned, continue with OIC"
echo "****************************************"

psm IntegrationCloud create-service -c templates/temp_psm_IntegrationCloud_Dev.json

echo "**************************\
 You can review the OIC Provisioning status with: psm [PAAS] operation-status -j [JOB_ID]"
echo "**************************"
