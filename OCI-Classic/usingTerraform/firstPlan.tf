...
### Storage ###
  ...
  resource "opc_compute_storage_volume" "my-volume-1" {
    size                = "20"
    description         = "my-volume-1: bootable storage volume"
    name                = "my-volume-1-boot"
...
### Compute ###
  ...
  resource "opc_compute_instance" "my-vm-instance-1" {
    ...
    label               = "my-vm-instance-1"
    shape               = "oc3"
    hostname            = "my-vm-instance-1"
    reverse_dns         = true
    storage {
      index             = 1
