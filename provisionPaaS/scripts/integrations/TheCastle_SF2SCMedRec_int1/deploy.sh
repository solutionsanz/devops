...
curl -u "${ICS_USERNAME}:${ICS_PASSWD}" -H "Content-Type:application/json" -X POST -d @${pathConnectors}/temp_${ICS_CONNECTOR_SC_CONFIG_NAME} ${ICS_CONNECTOR_SC_URI} -v
...
curl -u "${ICS_USERNAME}:${ICS_PASSWD}" -H "Content-Type:application/json" -X POST -d @${pathConnectors}/temp_${ICS_CONNECTOR_SF_CONFIG_NAME} ${ICS_CONNECTOR_SF_URI} -v


#	4.3 Anki-MedRec Connector

# Configure Anki-MedRec Connector:
curl -u "${ICS_USERNAME}:${ICS_PASSWD}" -H "Content-Type:application/json" -X POST -d @${pathConnectors}/temp_${ICS_REST_ANKIMEDREC_CONFIG_NAME} ${ICS_REST_ANKIMEDREC_URI} -v


#	4.4 APIs4Notifications Connector

# Configure APIs4Notifications Connector:
curl -u "${ICS_USERNAME}:${ICS_PASSWD}" -H "Content-Type:application/json" -X POST -d @${pathConnectors}/temp_${ICS_REST_APIs4Notifications_CONFIG_NAME} ${ICS_REST_APIs4Notifications_URI} -v
