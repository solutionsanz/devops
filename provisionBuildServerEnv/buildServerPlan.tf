...
### Null-Resources ###
  ...
  resource "null_resource" "my-vm-instance1" {
      depends_on = ["opc_compute_instance.my-vm-instance-1"]
      provisioner "file" {
        connection {
          timeout = "10m"
          type = "ssh"
      ...
      provisioner "remote-exec" {
        ...
        inline = [
          "chmod +x /tmp/*.sh",
          "sed -i 's/\r//g' /tmp/install_terraform.sh",
          "sed -i 's/\r//g' /tmp/install_psmcli.sh",
          "sudo /tmp/install_terraform.sh",
          "sudo /tmp/install_psmcli.sh ${var.ociUser} ${var.ociPass} ${var.idDomainName}",
          "psm setup -c /tmp/${var.psm_setup} && sudo rm /tmp/${var.psm_setup}"
        ]
      }
  }
